...
uint16_t DateTime::incYear()
{
  return ++yOff;
}

uint16_t DateTime::decYear()
{
  return --yOff;
}

int8_t DateTime::incMonth()
{
  if (++m == 13)
  {
    incYear();
...
int8_t DateTime::incDay()
{
  const int8_t days = pgm_read_byte(daysInMonth + m - 1);
  if (++d == days+1)
  {
    d = 1;
    incMonth();
