...
const uint8_t monthDays[] PROGMEM = { 31,28,31,30,31,30,31,31,30,31,30,31 }; //has to be const or compiler compaints

uint8_t DateTime::daysInMonth(const uint8_t m)
{
  return pgm_read_byte(monthDays + m - 1);
}
...
static uint16_t date2days(uint16_t y, uint8_t m, uint8_t d) {
    ...
    for (uint8_t i = 1; i < m; ++i)
        days += pgm_read_byte(monthDays + i - 1);
    if (m > 2 && y % 4 == 0)
        ++days;
...
DateTime::DateTime (uint32_t t) {
  t -= SECONDS_FROM_1970_TO_2000;    // bring to 2000 timestamp from 1970
    ...
    for (m = 1; ; ++m) {
        uint8_t daysPerMonth = pgm_read_byte(monthDays + m - 1);
        if (leap && m == 2)
            ++daysPerMonth;
...
bool DateTime::isDST(void) const {
  ...
  {
    const uint8_t days = pgm_read_byte(monthDays + m - 1); // sunday is 0
